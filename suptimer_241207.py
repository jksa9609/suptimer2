...
connection = mysql.connector.connect(
    host="localhost",      # DB 서버의 호스트
    user="root",       # DB 사용자명
    password="1234",   # DB 비밀번호
    database="schema_test1"  # 사용할 데이터베이스 이름
)
...
def fetch_data():
    # 데이터베이스에서 모든 데이터를 선택
    cursor.execute("SELECT * FROM table_test1")
    results = cursor.fetchall()  # 모든 결과를 가져옴
    
    # Listbox에 데이터를 추가
    for row in results:
        # row[1]은 title이라고 가정하고, Listbox의 끝에 데이터를 추가
        addlist1.insert("end", f"{row[1]}")  # "end"는 리스트의 마지막 인덱스를 의미

def delete_last_row():
    # 마지막 행의 ID를 찾기 위한 쿼리
    cursor.execute("SELECT MAX(todo_id) FROM table_test1")
    last_id = cursor.fetchone()[0]  # 마지막 ID 가져오기

    if last_id is not None:  # 마지막 ID가 존재하는 경우
        # 마지막 행 삭제
        cursor.execute("DELETE FROM table_test1 WHERE todo_id = %s", (last_id,))
        connection.commit()  # 변경사항을 DB에 적용    
        infolabel2.config(text=("행 제거 완료."))
        infolabel2.update()
    else:
        infolabel2.config(text=("삭제할 행 없음."))
        infolabel2.update()
...
def addtask():
    global todo_item2_2, todo_item3_2, e, file_path, memobox2, i, todo_item, todo_item2, todo_item3, todo_item4, i, running, sum1, todo_item2_int, todo_item3_int, memobox1, ll, title,  playtime_min,  playtime_sec, resttime_min, resttime_sec, contents
    running = True
    e += 1
    ...
    if todo_item and i < 20:
        todo_items.append((todo_item, sum1, todo_item2_int,
                          todo_item3_int, todo_item4))
        addlist1.insert(i, todo_item)
        i += 1

def insert_data():
    # 현재 시간을 DATETIME 형식으로 변환
    savetime = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]  # 소수점 이하 3자리까지
    todo_item
    sql = """INSERT INTO table_test1 
                (title, playtime_min, playtime_sec, resttime_min, resttime_sec, contents, savetime) 
                VALUES (%s, %s, %s, %s, %s, %s, %s)"""
    values = (todo_item, todo_item2_int, 0, todo_item3_int, 0, todo_item4, savetime)  # playtime_sec와 resttime_sec은 0으로 설정
...
def update_label(selected_index):
    if selected_index < len(todo_items):
        todo_item, sum1, todo_item2_int, todo_item3_int, todo_item4 = todo_items[
            selected_index]
...
def start2():
    ...
    myThread = threading.Thread(
        target=start_timer2, daemon=True)  # 타이머를 별도의 스레드에서 실행
    myThread.start()
...
addbutton1.place(anchor=tk.NW, width=50, height=40, x=270, y=70)

addbutton2 = tk.Button(root, bd=1, relief=tk.SOLID, text="행저장", command=insert_data)
addbutton2.place(anchor=tk.NW, width=50, height=40, x=340, y=70)

addbutton1 = tk.Button(root, bd=1, relief=tk.SOLID, text="행열기", command=fetch_data)
addbutton1.place(anchor=tk.NW, width=120, height=40, x=270, y=120)

# 버튼 세트 2 - 선택 시작, 모두 시작, 중지, 제거

# startbutton - 선택 시작
startbutton = tk.Button(root, bd=1, relief=tk.SOLID,
                        text="선택 시작", command=start1)
startbutton.place(anchor=tk.NW, width=110, height=40, x=270, y=200)

# startbutton2 - 모두 시작
startbutton2 = tk.Button(root, bd=1, relief=tk.SOLID,
                         text="모두 시작", command=start2)
...
# deletebutton1 - 제거
deletebutton1 = tk.Button(root, bd=1, relief=tk.SOLID,
                          text="제거", command=delete1)
deletebutton1.place(anchor=tk.NW, width=50, height=40, x=330, y=300)



presetbtn1 = tk.Button(root, bd=1, relief=tk.SOLID,
                       text="행제거", command=delete_last_row)
presetbtn1.place(anchor=tk.NW, width=50, height=40, x=430, y=410)


# 버튼 세트 3 - 메모 저장, 종료

# memobutton1 - 메모 저장
memobutton1 = tk.Button(root, bd=1, relief=tk.SOLID,
                        text="메모 저장", command=addmemo)
memobutton1.place(anchor=tk.NW, width=100, height=110, x=400, y=490)

# exitbutton - 종료(애플리케이션 종료)
exitbutton = tk.Button(root, bd=1, relief=tk.SOLID,
                       text="종료", command=shutdown1)
